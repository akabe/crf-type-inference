  ...
  let rec aux ctx e = match e.desc with
    ...
    | Eif (e1, e2, e3) ->
      ...
      List.map
        ...
        types
  in
  let z = csum snd (aux [] e) in
  match classify_float z with
  | FP_infinite | FP_nan -> failwith "divergence"
  | _ -> 1.0 /. z
...
module GibbsSampling = struct
  ...
  let select rng pdist =
    let rec aux z = function
      | [] -> failwith "select: empty distribution"
      | [(_, x)] -> x
      | (p, x) :: t -> if p > z then x else aux (z -. p) t
    in
    let s = List.fold_left (fun acc (p, _) -> acc +. p) 0. pdist in
    aux (Gsl.Rng.uniform rng *. s) pdist

  let gibbs_sampling1 ~rng ~types pf e =
    let update e =
      List.map (fun typ -> let e' = { e with typ } in (pf e', e')) types
      |> select rng
    in
    let rec aux ctx e = match e.desc with
      | Etrue | Efalse -> update e
      | Evar x -> { desc = Evar x; typ = List.assoc x ctx; }
      | Eabs (x, t, e1) ->
        let { typ = u; _ } = update { desc = Evar x; typ = t; } in
        update { e with desc = Eabs (x, u, aux ((x, u) :: ctx) e1); }
      | Eapp (e1, e2) ->
        update { e with desc = Eapp (aux ctx e1, aux ctx e2); }
      | Eif (e1, e2, e3) ->
        update { e with desc = Eif (aux ctx e1, aux ctx e2, aux ctx e3); }
    in
    aux [] e

  let fold ~rng ~sampler ~types pf f init e =
    let n = List.length types in
    let acc = ref init in
    for _ = 1 to sampler.gs_seqs do
      let r = ref (map_type
                     (fun _ -> List.nth types (Gsl.Rng.uniform_int rng n)) e) in
      for _ = 1 to sampler.gs_burn_in do
        r := gibbs_sampling1 ~rng ~types pf !r
      done;
      for _ = 1 to sampler.gs_samples do
        for _ = 1 to sampler.gs_period do
          r := gibbs_sampling1 ~rng ~types pf !r
        done;
        acc := f !acc !r
      done
  ...
  let grad_log_likelihood ~rng ~sampler ~types ff es w =
    let dJ_dw = Vec.make0 (Vec.dim w) in
    let alpha = (-1.) /. float (sampler.gs_samples * sampler.gs_seqs) in
    let aux e =
      axpy (graph_feature ff e) dJ_dw;
      fold ~rng ~sampler ~types (vertex_potential ff w)
        (fun () e' -> axpy ~alpha (graph_feature ff e') dJ_dw) () e;
    in
    List.iter aux es;
    dJ_dw

  let grad_log_posterior ~rng ~sampler ~types ~sigma2 ff es w =
    let dJ_dw = grad_log_likelihood ~rng ~sampler ~types ff es w in
    axpy ~alpha:((-1.) /. sigma2) w dJ_dw;
    dJ_dw

  let normalizer ~rng ~sampler ~types ff w e =
    let x =
      fold ~rng ~sampler ~types (vertex_potential ff w)
        (fun acc e' -> acc +. 1. /. graph_potential ff w e') 0. e in
    let b = float (List.length types) ** float (Lambda.size e) in
    let c = float (sampler.gs_samples * sampler.gs_seqs) in
    x /. (b *. c)

  let prob ~rng ~sampler ~types ff w e =
    graph_potential ff w e *. normalizer ~rng ~sampler ~types ff w e

  let likelihood ~rng ~sampler ~types ff es w =
    let p = prob ~rng ~sampler ~types ff w in
    List.fold_left (fun acc e -> acc +. p e) 0.0 es

  (** {3 S} *)

  let graph_feature_switch ~c1 ~c2 ~dim ff et eb =
    let x = Vec.make0 dim in
    let rec aux et eb =
      let alpha = if eb.typ then c1 else c2 in
      axpy ~alpha (ff et) x;
      match et.desc, eb.desc with
      | Etrue, Etrue | Efalse, Efalse | Evar _, Evar _ -> ()
      | Eabs (x, t, et1), Eabs (y, u, eb1) ->
        aux { desc = Evar x; typ = t; } { desc = Evar y; typ = u; };
        aux et1 eb1;
      | Eapp (et1, et2), Eapp (eb1, eb2) ->
        aux et1 eb1;
        aux et2 eb2;
      | Eif (et1, et2, et3), Eif (eb1, eb2, eb3) ->
        aux et1 eb1;
        aux et2 eb2;
        aux et3 eb3;
      | _ -> assert false
  ...
  let grad_log_likelihood_switch
      ?(c1 = 0.0) ?(c2 = 1.0)
      ~rng ~sampler ~types ff es w =
    let dJ_dw = Vec.make0 (Vec.dim w) in
    let alpha = (-1.) /. float (sampler.gs_samples * sampler.gs_seqs) in
    let gff = graph_feature_switch ~c1 ~c2 ~dim:(Vec.dim w) ff in
    let aux et =
      let eb =
        let (e2, _) = Lambda.untyping et
                      |> infer ~types (vertex_log_potential ff w) in
        let e1 = SimpleType.typing_with_hints [] e2 in
        let bindings = ref [] in
        Lambda.map2_type (SimpleType.unifiable bindings) e1 e2 in
      axpy (gff et eb) dJ_dw;
      fold ~rng ~sampler ~types (vertex_potential ff w)
        (fun () e' -> axpy ~alpha (gff e' eb) dJ_dw) () et;
    in
    List.iter aux es;
    dJ_dw
